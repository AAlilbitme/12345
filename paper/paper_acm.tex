...
We identified a category error causing two independent soundness gaps: modeling one-shot on-chain resources (HTLC outputs and revoked commitment outputs) as Tamarin \textbf{persistent facts} rather than \textbf{linear facts}. A persistent fact is never consumed, so a rule gated only by such a fact may fire arbitrarily often; when that fact stands for an output that can be spent exactly once, the encoding silently permits multiple spends.

\textbf{Instance 1: HTLC Output Duplication.} The forwarding rules were gated only by a public HTLC-offer message. A Dolev--Yao replay of that offer could re-fire an honest forward, locking a second HTLC on the same channel output with an adversary-chosen amount. We replaced a global \texttt{OneHTLCPerPtr} axiom with a structural guard: each channel open emits one linear \texttt{Free(ptr)} token, and every HTLC-add consumes it.

\textbf{Instance 2: Revoked Commitment Replay.} The punishment rules were gated only by a persistent \texttt{!RevokedSecret} fact. Because persistent facts are never consumed, the punishment rule could fire arbitrarily many times for the same revoked state, allowing repeated fund extraction from a single cheat. We fixed this by emitting a linear \texttt{RevokedCommitmentUnspent} token in the revocation rule, which is consumed upon punishment. Persistent \emph{knowledge} of the revocation secret is retained via \texttt{!RevokedSecret}; only the one-shot \emph{spend} is made linear.

We stress that Instance~2 is a consequence of integration rather than of scale. It resides at the junction of the revocation lifecycle and the routing execution: the offending rule is gated by revocation state yet re-fired along a routing trace. A model containing only the two-party channel layer, or only an abstracted forwarding layer, does not exhibit the trace that exposes it. This is the sense in which the breadth of the model is a precondition for the finding rather than an end in itself.

After these changes, ``at most one spend per output'' is a consequence of linear-fact consumption—one fewer assumption. Adding an input fact restricts a rule's firings and therefore preserves all-traces safety properties automatically; it can, however, invalidate an exists-trace witness, so the theory was fully re-run rather than argued by monotonicity. All 43 lemmas re-verify after the change (reported: 262 s, Tamarin 1.8).

\textbf{The reusable invariant.} Both instances share one root cause and one remedy, which we state as a modeling discipline: \emph{every one-shot on-chain output must be modeled as a linear fact}. Persistent facts may encode durable \emph{knowledge} (keys, revealed secrets, channel identity), but a \emph{spend opportunity} must be tracked by a linear token consumed on use. This yields a mechanical audit question for symbolic payment-channel models: for each on-chain output that ought to be single-spend, is there a linear fact consumed when it is spent?
...
\textbf{Refinement Theorem (Pen-and-Paper).} Internalising the network removes Dolev--Yao power, so the abstract model $A$ has strictly fewer behaviours than the concrete model $C$; abstract safety therefore does not by itself imply concrete safety. The gap is exactly the adversary's ability to forge or replay a forward, and is closed by a \emph{local} per-hop authentication lemma AUTH, machine-checked in $C$ (\texttt{Forward1\_Requires\_Offer}, 266 steps; \texttt{Forward2\_Requires\_Forward1}, 274 steps), which quantifies over a single adjacent pair and never over the path or $N$. Under AUTH, any safety property $\varphi$ holding in $A$ for all $N$ holds in $C$ for all $N$, by projection and induction over forwarding steps. The refinement argument itself is \textbf{pen-and-paper}; Tamarin cannot quantify over $N$ or relate two theories. This is the honest boundary of our verification: both hypotheses are machine-checked, the composition is not.
