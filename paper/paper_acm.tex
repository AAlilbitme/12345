...
\begin{enumerate}
    ...
    \item \textbf{A modular, fully machine-checked model} integrating the two-party channel lifecycle (including revocation and punishment) with multi-hop HTLC routing and fees. It establishes 56 distinct properties; the total of 81 lemma checks includes 25 properties re-proved over the $N$-hop abstraction, and we report this composition explicitly in Table~\ref{tab:theories} rather than quoting a single aggregate.
...
\centering
\small
\caption{The five-theory modular architecture. The first four theories establish 56 distinct properties. The $N$-hop theory does not add properties: each of its 25 lemmas restates a property already proved in \texttt{multihop.spthy}, re-established over the abstract routing layer, giving 81 lemma checks in total.}
\label{tab:theories}
...
Fees are enforced structurally in the $N$-hop model as a firing guard (\texttt{Eq(\%vIn, \%vOut + \%fee)}). Because end-to-end conservation over $N$ hops is a sum of $N$ fees, it is not finitely statable in one Tamarin lemma without quantifying over $N$.
