...
% Palette shared by all tikz figures (see paper/figures.tex)
\definecolor{honestgreen}{RGB}{46,125,50}
\definecolor{timeoutamber}{RGB}{239,108,0}
\definecolor{attackred}{RGB}{198,40,40}
\definecolor{persistentgray}{RGB}{97,97,97}
\definecolor{linearblue}{RGB}{21,101,192}
\definecolor{corefill}{RGB}{227,242,253}   % pale blue
\definecolor{timingfill}{RGB}{243,229,245} % pale purple
\definecolor{extfill}{RGB}{232,245,233}    % pale green
...
\toprule
\textbf{Theory File} & \textbf{Aspect Modeled} & \textbf{Safety} & \textbf{Witness} & \textbf{Time} & \textbf{Builtins} \\
\midrule
\texttt{multihop.spthy} & Lifecycle, 3-hop routing, value/fees, wormhole & 33 & 10 & 353.31\,s & Hashing, Signing, Nat \\
\texttt{Clock.spthy} & Block-clock lifecycle \& staggered-CLTV claim windows & 6 & 3 & 54.37\,s & Natural-numbers \\
\texttt{Cltv.spthy} & Pure CLTV block arithmetic (delta positivity) & 3 & 0 & 0.23\,s & Natural-numbers \\
\texttt{timeout.spthy} & Early-timeout race (T2b removed) & 0 & 1 & 0.06\,s & --- \\
\midrule
\multicolumn{2}{@{}l}{\emph{Distinct properties}} & \textbf{42} & \textbf{14} & & \\
\midrule
\texttt{Modif.spthy} & \emph{Re-proof} of the above over the $N$-hop abstraction & 19 & 6 & 134.39\,s & Hashing, Signing, Multiset \\
\bottomrule
\end{tabular}
\end{table*}

\subsection{Tactic Engineering and Reproduction}
All results were reproduced in a single sitting on one machine under Tamarin 1.10.0 with Maude 3.1; every theory passes Tamarin's well-formedness checks, and the whole package verifies in 542.36\,s of wall-clock time. Results were produced under a strict test-first discipline: a property is claimed only after Tamarin reports it. Per-lemma invocation is used on the large theory to avoid OOM. The flag \texttt{-{}-stop-on-trace=seqdfs} rescues the heavy existence witnesses but hangs the natural-number arithmetic theories, so it is applied selectively.
