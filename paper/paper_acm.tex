...
prover, structured as a modular package of five theories establishing
\textbf{56 distinct machine-checked properties} (81 lemma checks in total, since
25 properties are additionally re-proved in the $N$-hop abstraction), covering
the two-party channel lifecycle, multi-hop HTLC routing with fees,
staggered-CLTV timing, and an $N$-hop abstraction.
...
discipline for symbolic payment-channel analyses. We further prove an end-to-end value-conservation theorem
...
\begin{enumerate}
    ...
    \item A modular, fully machine-checked model integrating the two-party channel lifecycle (including revocation and punishment) with multi-hop HTLC routing and fees, establishing \textbf{56 distinct properties} across five theories (81 lemma checks, as 25 properties are re-proved in the $N$-hop abstraction). This integration is the precondition for the finding above. We report the composition of this count explicitly in Table~\ref{tab:theories} rather than quoting a single aggregate.
...
\end{itemize}
These are not on an equal footing. T1 and T2a/T2b are \emph{passive temporal
orderings}: they constrain when events may occur relative to one another, and
correspond to the CLTV structure that Section~6 derives independently from a
concrete block clock. T3 is different in kind---it obliges an intermediary to
\emph{act}, sweeping its incoming HTLC once the outgoing one is redeemed. It is
therefore the assumption most at risk of being an unnoticed strengthening, and
the one we subject to minimality testing in Section~\ref{sec:verification}. We
rely on all four for our main theorems and do not claim minimality for the
passive orderings. This mirrors the real-world necessity of LN watchtowers~\cite{mccorry2019pisa}. Our core security goals translate directly to verified lemmas: Payment Atomicity, Intermediary Safety, Routing Causality \& Authentication, Timing Safety, and Value Conservation.
...
\centering
\small
\caption{The five-theory modular architecture. ``Safety'' counts all-traces
properties and ``Witness'' counts exists-trace lemmas (reachability results,
attack witnesses, and non-vacuity checks). The $N$-hop theory re-proves
properties already counted in \texttt{multihop.spthy} rather than adding new
ones, so the distinct total is 56 and the total number of lemma checks is 81.}
\label{tab:theories}
\begin{tabular}{@{}llccl@{}}
\toprule
...
\midrule
\texttt{multihop\_nhop.spthy} & \emph{Re-proof} of the above in the $N$-hop abstraction & 19 & 6 & Hashing, Signing, Nat \\
\bottomrule
\end{tabular}
\end{table*}
...
Across the theories of Table~\ref{tab:theories} we establish 56 distinct
properties (81 lemma checks in total). We stress that the $N$-hop theory
contributes no new properties: every one of its 25 lemmas restates a property
already proved in \texttt{multihop.spthy}, re-established over the abstract
routing layer. Of the 43 lemmas of the concrete theory, 10 are exists-trace
witnesses (including non-vacuity checks and the attack traces of
Sections~\ref{sec:discipline} and 6.2) and 5 are \texttt{[reuse]} helper lemmas
that support other proofs rather than stating protocol goals; the complete
lemma index is given in the artifact. Table~\ref{tab:lemmas} maps the lemma groups in our primary concrete theory.
...
The following observation is immediate from the transition relation---a
persistent premise is checked but not removed---and we state it only to fix the
shape of the defect precisely. The contribution is \emph{not} this fact about
Tamarin, which is standard, but the identification of which Lightning constructs
fall under Definition~\ref{def:oneshot} and what the concrete exploit is when
they do not.
...
\begin{proof}
Immediate: firing $ri$ removes only $\mathrm{lin}(l)$, and
$!\mathsf{Res}(\bar t) \in \Sigma_{\mathrm{per}}$ is checked but not removed, so
with $P$ re-derivable the enabling condition holds again in the successor state.
\end{proof}

\begin{remark}[Where the difficulty actually lies]
\label{rem:notmath}
That linear facts model consumable resources is elementary, and experienced
Tamarin users apply it routinely to nonces, session keys, and state-machine
transitions. The difficulty is not the mechanism but the \emph{mapping}: deciding
which constructs of a payment-channel protocol denote one-shot on-chain outputs.
This is not syntactically evident. A revoked commitment presents itself in the
protocol description as \emph{knowledge}---``the counterparty learns the
revocation secret''---and knowledge is exactly what persistent facts are for; the
natural encoding is therefore the wrong one. What is easy to miss is that the
same revocation event confers two distinct things: durable knowledge of the
secret, and a single opportunity to spend the revoked output. Our contribution
is the identification of this split, and the demonstration below that getting it
wrong yields not a modeling infelicity but an exploitable trace.
\end{remark}
...
\medskip
\noindent\textbf{Instance 2: revoked commitment replay.} The punishment rules
were gated only by the persistent $!\mathsf{RevokedSecret}$ fact, and are thus an
instance of Proposition~\ref{prop:bug}. The consequence is concrete rather than
cosmetic. A revoked commitment corresponds to one on-chain transaction spending
one funding output; the counterparty's penalty claim spends the resulting output
once. Under persistent gating, however, a single instance of cheating licenses
$\mathsf{PublishRevoked}$---and hence $\mathsf{Punish}$---arbitrarily many times
for the same revoked state $n$, so the model admits an execution in which the
honest party confiscates the channel balance repeatedly from one cheat. A model
in which punishment is unbounded does not merely overstate the honest party's
position: any downstream analysis of balance security, of griefing economics, or
of watchtower incentives inherits an adversary-favourable or victim-favourable
distortion depending on which party is compromised. The defect is silent because
every individual lemma about punishment soundness
(\texttt{No\_Punishment\_Without\_Cheating}) continues to hold---punishment still
requires a prior cheat; it is only the \emph{multiplicity} that is wrong.
...
The wormhole attack~\cite{malavolta2019anonymous} occurs when two colluding path members relay the preimage out-of-band, skipping an honest intermediary who forwards value but earns no fee. It is an instance of the agent-skipping class formalized symbolically by Smith
et al.~\cite{smith2022agentskipping}, and we claim no novelty for exhibiting it.
Its role here is to discharge a specific obligation created by
Section~\ref{sec:discipline}. Having constrained the model with two linear
single-spend tokens, we must show that the repair is \emph{sound} rather than
merely restrictive: a guard that eliminates the double-punishment trace by
over-constraining the protocol would also eliminate legitimate executions, and
the resulting model would verify safety vacuously. Reproducing the wormhole
demonstrates the opposite---the hardened model still admits the known economic
pathologies of HTLC routing, so the linear tokens remove exactly the spurious
multiplicity and nothing else. It is, in this sense, a non-vacuity argument for
the central contribution rather than an attack result in its own right. In the symbolic model, the Dolev--Yao network is the colluders' side channel. We machine-check it twice: \texttt{Wormhole\_Fee\_Theft\_Reachable} (51 steps) exhibits a trace where the payment settles yet the middle hop is never redeemed and no intermediary earns a fee, with no key compromise; and \texttt{Wormhole\_Steals\_Exactly\_The\_Fees} (81 steps) quantifies the theft to precisely the path fees \texttt{\%f1 + \%f2}, tying the attack to the value layer of a model that simultaneously carries channel revocation and settlement. Known mitigations (AMHL, PTLCs) are out of scope.

\subsection{Economic Soundness and Griefing}
\texttt{EndToEnd\_Value\_Conservation} establishes that the sender locks the
receiver's amount plus the accumulated fees, so the routing mechanism neither
creates nor destroys value. As a reachability result,
\texttt{Honest\_Intermediary\_Refunds} exhibits an honest intermediary induced
to lock capital and then refund on timeout---the standard PCN griefing DoS.

\begin{remark}[Zero-fee forwarding is out of scope]
\label{rem:zerofee}
The lemmas \texttt{Fee\_Strictly\_}\texttt{Positive\_Hop1/2} hold in our model
for a reason that is an artifact of the encoding rather than a property of Lightning:
Tamarin's \texttt{natural-numbers} sort provides no $\%0$ term, so a zero-fee
forward is not expressible. We therefore do \emph{not} present these as economic
theorems. Deployed Lightning explicitly permits zero base fees, which nodes use
for rebalancing and probing; our model consequently verifies a \emph{restricted
subset} of the protocol in which every hop charges a positive fee, and cannot
evaluate routing strategies that depend on zero-fee forwards. Admitting zero
fees requires either a different arithmetic encoding or an explicit zero
constant with the associated case analysis, which we leave to future work. We
note that the value-conservation and atomicity results do not depend on fee
positivity; only the fee-specific lemmas do.
\end{remark}
...
Among our four restrictions, T3 is the one that encodes an obligation to act
rather than a temporal ordering, and is accordingly the one whose necessity is
least self-evident: it is natural to suspect it is implied by the orderings T1
and T2b. We tested it by removing T3 and re-running the suite: \texttt{Intermediary\_Never\_Loses\_Under\_Liveness} is falsified, with Tamarin finding a concrete 59-step counterexample. T2b orders the two timeouts but does not force the node to sweep the incoming HTLC once its outgoing one is redeemed; that action is exactly what T3 encodes. T3 is therefore \textbf{load-bearing} for intermediary safety: it is not
redundant given the temporal orderings, because no ordering constraint can
compel an action. We tested the removal of T3 specifically because it is the
only one of the four that is an obligation rather than an ordering; we make no
claim regarding the individual minimality of T1, T2a, or T2b, whose content is
independently corroborated by the block-clock derivation of Section~6.

% ==============================================================
\section{\texorpdfstring{An Abstraction for Unbounded Path Length ($N$-Hops)}{An Abstraction for Unbounded Path Length (N-Hops)}}
\label{sec:nhop}
% ==============================================================
...
To bypass this, we abstract the network layer into an internal Tamarin linear
fact---\texttt{Route(prev, me, ptr, y)}---handed directly from one hop to the
next. Because the HTLC is no longer a self-referential signed term, Tamarin's
origin analysis no longer recurses.

We are deliberate about what this buys and what it costs. Internalising the
network \emph{removes the Dolev--Yao adversary from the routing layer}: in the
abstract model the attacker can no longer inject, replay, or forge a forwarded
HTLC. What the abstract model therefore establishes at unbounded $N$ is that the
\emph{internal state machine} of routing---the discipline of linear HTLC slots,
the pairing of redeem and refund outcomes, and the timing restrictions---remains
consistent along a chain of any length. It does \emph{not} establish
network-level adversarial security at $N$ hops; that content resides entirely in
the concrete model and reaches arbitrary $N$ only through the pen-and-paper
argument of Theorem~\ref{thm:refine}, whose status we delimit in
Remark~\ref{rem:scope}. Readers should regard this section as a tractability
workaround with a precisely stated transfer condition, not as a machine-checked
security result for unbounded path length. This yields \texttt{multihop\_nhop.spthy} (25 active lemmas; 4 \texttt{[use\_induction]} lemmas are commented out as deliberate tractability boundaries because backward chaining through unbounded \texttt{Route} facts causes divergence).
...
We delivered a modular, machine-checked model that integrates the Lightning two-party channel lifecycle with multi-hop HTLC routing and fees (56 distinct properties over five theories). That integration surfaced a \emph{linear single-spend discipline}: two independent soundness gaps arising from encoding one-shot on-chain outputs as persistent facts, repaired uniformly by linear tokens, and generalizable into an auditable modeling invariant. We further established the minimality of the T3 timing assumption empirically, proved end-to-end value conservation, exposed a griefing DoS, reproduced the known wormhole attack as a fidelity check, and mapped where the prover stops scaling.
