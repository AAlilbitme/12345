...
Smith et al.~\cite{smith2022modelling} develop a general symbolic framework, in the multiset-rewriting setting, for agent-skipping attacks on message-forwarding protocols, introducing path-integrity goals and analyzing several protocols including payment channel networks. The wormhole attack on HTLC routing~\cite{malavolta2019anonymous} is an instance of agent-skipping. Our treatment of the wormhole is therefore not a novel attack finding but a fidelity check: we confirm that our integrated model reproduces the known attack, tying it to the value layer by quantifying the theft to exactly the bypassed routing fees. This demonstrates that our strict linear single-spend fix prevents unbounded punishment without falsely blocking valid, albeit adversarial, routing behaviors like the wormhole.
...
The wormhole attack~\cite{malavolta2019anonymous} is an instance of the agent-skipping class formalized symbolically by Smith et al.~\cite{smith2022modelling}. We claim no novelty for exhibiting it. Its role here is to discharge an obligation created by Section~\ref{sec:discipline}: having constrained the model with two linear single-spend tokens, we must show the repair is \emph{sound} rather than merely restrictive, since a guard that eliminated the unbounded-punishment trace by over-constraining the protocol would also eliminate legitimate executions and render subsequent safety results vacuous.
