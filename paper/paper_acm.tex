...
The relationship to our work is one of complementary abstraction, and the boundary is sharp. Their signature comprises hashing, pairing, encryption and signatures, with sorts \texttt{msg}, \texttt{pub}, \texttt{fresh} and \texttt{Fact}; there is no arithmetic and no numeric sort, and their security goals are correspondence properties over the ordering of \texttt{Add} and \texttt{Forward} events. Value therefore never enters the predicate: routing fees are discussed in prose as the real-world consequence that makes path integrity worth having, but no balance, amount, or conservation property is modelled. Consequently the attack is establishable in their framework as a \emph{skipped hop}, whereas its \emph{cost} is not expressible.
