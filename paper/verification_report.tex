...
\texttt{Modif.spthy} & $N$-hop generalisation (generic linear-fact forward)    & 25 & hashing, signing, nat \\
\texttt{Clock.spthy}         & block-clock lifecycle \& staggered-CLTV claim windows   & 9  & natural-numbers \\
\texttt{Cltv.spthy}   & pure CLTV block arithmetic (delta positivity)           & 3  & natural-numbers \\
...
value/fee conservation could be merged into it. \texttt{Modif.spthy}
...
\subsection{Timing safety (\texttt{Cltv}, \texttt{Clock}, \texttt{timeout})}

Figure~\ref{fig:timing} shows the staggered-timelock story, split across three
small natural-number theories. \texttt{Cltv} \emph{derives} the CLTV-delta
...
The linear-fact route yields \texttt{Modif.spthy}, which carries amounts
...
python3 run.py Modif.spthy  # the N-hop generalisation (25 lemmas)
\end{lstlisting}
...
carries the three timing theories (\texttt{Cltv}, \texttt{Clock},
