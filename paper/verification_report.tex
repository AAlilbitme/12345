...
\midrule
\texttt{multihop.spthy}    & channel lifecycle + routing + value/fees + atomicity & 43 & hashing, signing, nat \\
...
\midrule
Signed-message generic forward & self-referential signed term $\Rightarrow$ Tamarin origin analysis recurses; even a one-hop witness OOMs & \rc \\
\;\; $+$ fresh per-hop id & fixes replay but \emph{not} termination---still a signed term producible only by another forward & \rc \\
Linear-fact routing (idealised channel) & safety invariants verify by induction over the chain, for any $N$ & \gc \\
\bottomrule
\end{tabular}
